...
BC_bool StringEqualImpl(const BCObjectRef a, const BCObjectRef b) {
  ...
  if (h1 != BC_HASH_UNSET && h2 != BC_HASH_UNSET && h1 != h2)
    return BC_false;

  if (BCStringLength(s1) != BCStringLength(s2))
    return BC_false;
...
BCObjectRef StringCopyImpl(const BCObjectRef obj) { return BCRetain(obj); }

// =========================================================
// MARK: Class
// =========================================================

static BCClass kBCStringClass = {.name = "BCString",
                                 .dealloc = NULL,
                                 .hash = StringHashImpl,
...
BCClassId BCStringClassId() { return kBCStringClass.id; }
...
void ___BCINTERNAL___StringPoolDeinitialize(void) {
  ...
  for (size_t i = 0; i < BC_STRING_POOL_SIZE; i++) {
    ...
    while (node) {
      ...
      node = next;
      if (next == NULL)
        StringPool.buckets[i] = NULL;
    }
  }
}

static BCStringRef StringPoolGetOrInsert(const char *text, const size_t len,
                                         const uint32_t hash,
  ...
  // ============================================
  // Insert
  const size_t extraAlloc = static_string ? sizeof(StringPoolNode)
                                          : sizeof(StringPoolNode) + (len + 1);
  const BCStringRef newStr = (BCStringRef)BCObjectAllocWithConfig(
      NULL, kBCStringClass.id, extraAlloc,
      static_string ? BC_STRING_FLAG_POOLED | BC_STRING_FLAG_STATIC
  ...
  StringPoolNode *newNode =
      (StringPoolNode *)((char *)newStr + sizeof(BCString) +
...
BCStringRef BCStringPooled(const char *text) {
  if (!text)
    return NULL;
  return StringPoolGetOrInsert(text, strlen(text),
                               ___BCINTERNAL___StringHasher(text), BC_false);
}

BCStringRef BCStringPooledWithInfo(const char *text, const size_t len,
                                   const uint32_t hash,
                                   const BC_bool static_string) {
  if (!text)
    return NULL;
...
const char *BCStringCPtr(const BCStringRef str) { return str->buffer; }
...
void BCStringPoolDebugDump(void) {
  ...
  printf("\n"
         "                                          " BOLD
         "String Pool Dump" RESET "\n"
         "┌"
         "────────┬────────────────────────────────────────────────┬───────────"
         "─┬────────┬──────────────────"
         "┐\n"
         "│" BLACK BOLD " Bucket " RESET BLACK "│" BOLD
         "                     Value                      " RESET BLACK "│" BOLD
         "    Hash    " RESET BLACK "│" BOLD " Length " RESET BLACK "│" BOLD
         "       Next       " RESET "│\n"
         "├" DGRAY "────────┼────────────────────────────────────────────────┼─"
                   "───────────┼────────┼──────────────────" RESET "┤\n");
